          ...
          <table className="w-full text-left text-xs font-mono min-w-[640px]">
            <thead>
              <tr className="text-zinc-500">
                ...
                <th className="py-3 px-3 font-normal">Sample (A / B)</th>
                <th className="py-3 px-3 font-normal">CVR (A &rarr; B)</th>
                <th className="py-3 px-3 font-normal">Relative Lift</th>
                <th className="py-3 px-3 font-normal">Interaction Test</th>
                <th className="py-3 px-3 font-normal text-right">Verdict</th>
              </tr>
            </thead>
            <tbody>
              {hteData.subgroups.map((sub) => (
                <tr key={sub.segment_name} className="hover:bg-white/[0.02] transition-colors">
                  <td className="py-3 px-3 font-medium text-white">{sub.segment_name}</td>
      ...
      <div className="rounded-2xl bg-white/[0.03] p-5 sm:p-6 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2">
          ...
          <div className="flex items-center space-x-3 text-xs">
            ...
            <span className="font-mono text-white font-bold">{fdrAlpha.toFixed(2)}</span>
            <input
              type="range"
              min="0.01"
              max="0.20"
          ...
          {guardrailsData.metrics.map((metric) => {
            ...
            return (
              <div
                ...
                className="rounded-xl bg-white/[0.02] p-4 transition-all hover:bg-white/[0.04]"
              >
                ...
                <div className="mt-3 flex items-baseline justify-between text-xs font-mono">
                  ...
                  <span className="text-white font-medium">Observed: {metric.treatment_value}</span>
                </div>
                ...
                <div className="mt-3 pt-2 flex items-center justify-between text-[11px] font-mono text-zinc-500">
                  ...
                  <span className="text-zinc-300">FDR p: {metric.adjusted_p_value.toFixed(3)}</span>
                </div>
              </div>
            );
          })}
